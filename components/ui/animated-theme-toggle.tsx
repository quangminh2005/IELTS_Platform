...
export const AnimatedThemeToggle = ({ className }: { className?: string }) => {
  ...
  // Đồng bộ trạng thái với theme thực tế đã được áp dụng (script chống nhấp nháy đặt sẵn).
  useEffect(() => {
...
const SolarSwitch = ({ isDark }: { isDark: boolean }) => {
  ...
  // scale + pathLength được điều khiển qua variants + `animate` của phần tử cha,
  // nên việc chuyển sáng↔tối luôn vẽ/ẩn đúng tia mặt trời và mặt trăng.
  const moonVariants = {
